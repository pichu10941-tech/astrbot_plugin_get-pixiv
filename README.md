...
为 AstrBot 提供 Pixiv / Booru 图片获取能力，并修复 NapCat/LLOB 发送 Pixiv 图片时的防盗链错误。
...
LLM 可从 danbooru 或 gelbooru 获取图片直链。需要在插件配置中填写对应站点的 API key。
...
在 AstrBot WebUI 的插件配置页面填写（均为可选）：

| 配置项 | 说明 |
|---|---|
| `danbooru_login` | danbooru.donmai.us 用户名 |
| `danbooru_api_key` | danbooru API Key（个人设置页面生成） |
| `gelbooru_user_id` | gelbooru.com 用户 ID |
| `gelbooru_api_key` | gelbooru API Key（个人设置页面生成） |

不填写 booru 配置时，Pixiv 功能仍然完整可用。
